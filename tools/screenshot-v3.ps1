# tools/screenshot-v3.ps1 - tira print da janela do app por PID (sem filtro de tamanho)
...
$callback = {
  ...
  if ($p -eq $pid_target) {
    ...
    $w2 = $r2.R - $r2.L
    $hi2 = $r2.B - $r2.T
    Write-Host "hwnd=$h title='$title' visible=$visible size=${w2}x${hi2}"
    if ($visible -and $w2 -gt 0 -and $hi2 -gt 0) {
      $script:hwnd_found = $h
    }
  }
  return $true
...
if ($script:hwnd_found -eq [IntPtr]::Zero) {
  Write-Output "nenhuma janela visivel"
  exit 1
}
...
$w = $r.R - $r.L
$hi = $r.B - $r.T
Write-Host "Tirando print: ${w}x${hi}"
