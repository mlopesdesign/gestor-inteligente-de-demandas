...
export function adiar(db, payload, sessao) {
  ...
  const r1 = db.exec(
    `UPDATE tarefas SET vencimento_em=?, adiada_ate=?, adiada_motivo=?, motivo_adiamento=?, status='ADIADA', atualizado_em=?, versao=versao+1 WHERE id=? AND usuario_id=? AND versão=?`,
