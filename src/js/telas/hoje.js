    ...
    <div class="main">
      ...
      <main class="conteudo">
        <div style="display:flex; align-items:center; gap:8px; margin-bottom:12px;">
          ...
          <button id="btn-cobranca" title="Cobrar tarefas atrasadas agora">🔔 Cobrar</button>
        </div>
...
function renderBuckets(tarefas) {
  ...
  html += sec('🔴 Críticas', buckets.criticas, 'var(--danger)');
  html += sec('⚠️ Atrasadas', buckets.atrasadas, 'var(--danger)');
  html += sec('📅 Vencendo hoje', buckets.hoje, 'var(--warning)');
  html += sec('📆 Amanhã', buckets.amanha, 'var(--warning)');
...
function renderLinha(t) {
  ...
  return `<li>
    ...
    <span style="color:var(--fg-3); font-size:11px;">${vencida ? '<span style="color:var(--danger);">⚠ ' : ''}${venc ? formatarVenc(venc) : '—'}</span>
